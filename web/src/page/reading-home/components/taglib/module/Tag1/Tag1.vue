<template>
    <div>
        <el-button type="primary" size="mini" round @click="addTagBox()">新增{{this.tagLevel}}级标签</el-button>
        <TagTable :fatherlist="fatherlist" :tagLevel="tagLevel" v-loading="tableLoading"></TagTable>
        <EditTag ref="editTag" :info="editInfo"></EditTag>
    </div>
</template>

<script>
    import TagTable from '../TagTable'
    import EditTag from '../EditTag'
    import Api from '@/service/api.js'
    import {cloneDeep} from '@/lib/util'

    export default {
        name: "Tag1",
        components: {TagTable, EditTag},
        data() {
            return {
                editInfo: {
                    tagInfo: {
                        father: {
                            id: '',
                            createtime: '',
                            name: '',
                            fid: '',
                            level: '',
                        },
                        childlist: []
                    },
                    allTags2: [],
                    func: '',
                },
                fatherlist: [],
                tagLevel: 1,
                tableLoading: true,
            }
        },
        mounted() {
            this.updateData()
        },
        methods: {
            /**
             * 拉取一级标签和二级标签,
             * 其中二级标签有fid=0的,是未绑定状态的标签, 要排除掉
             * @return {Promise<void>}
             */
            async updateData() {
                const res = await Api.get('/oa/call', {
                    func: 12,
                    cgigettagfatherlistreq: JSON.stringify({
                        level: this.tagLevel
                    })
                })
                this.fatherlist = res.fatherlist
                const res2 = await Api.get('/oa/call', {
                    func: 11,
                    gettaginforeq: JSON.stringify({
                        level: 2
                    })
                })
                this.editInfo.allTags2 = []
                res2.taglist.map(item => {
                    // fid不为0的是绑定后插入的标签
                    if (item.fid !== 0) {
                        return
                    }
                    this.editInfo.allTags2.push({
                        id: item.id,
                        value: item.name
                    });
                })

                this.tableLoading = false
            },
            // 向服务器发送添加标签的信息
            async addTag(param) {
                if (this.checkName(param)) {
                    await Api.post('/oa/call', {
                        func: 9,
                        addtaginforeq: JSON.stringify({
                            name: param.name,
                            level: this.tagLevel,

                        })
                    })
                    this.updateData()
                }
            },
            // 向服务器发送删除标签的信息
            async deleteTag(param) {
                await Api.post('/oa/call', {
                    func: 10,
                    deletetaginforeq: JSON.stringify({
                        ...param
                    })
                })
                this.updateData()
            },
            // 向服务器发送修改的标签信息,并重新拉取数据
            async updateTag(param) {
                if (this.checkName(param)) {
                    await Api.post('/oa/call', {
                        func: '13',
                        cgiupdatetaginforeq: JSON.stringify({
                            ...param
                        })
                    })
                    this.updateData()
                }
            },
            // 弹出修改标签box
            updateTagBox(index, row) {
                this.editInfo.tagInfo = cloneDeep(this.fatherlist[index])
                this.editInfo.func = 'editTag'
                this.$refs.editTag.show()
            },
            // 弹出新建标签box
            addTagBox() {
                this.editInfo.tagInfo = {
                    father: {
                        id: '',
                        createtime: '',
                        name: '',
                        fid: '',
                        level: this.tagLevel,
                    },
                    childlist: []
                }
                this.editInfo.func = 'addTag'
                this.$refs.editTag.show()
            },
            // 标签排重
            checkName(param) {
                const has = this.fatherlist.some(item => {
                    if (item.father.name === param.newtagname || item.father.name === param.name) {
                        if (param.id) {
                            return item.father.id !== param.id;
                        } else {
                            return true
                        }
                    }
                })
                if (has) {
                    this.$message({
                        type: 'error',
                        message: `标签: ${param.newtagname || param.name} 已存在!`
                    })
                }
                return !has
            }
        }
    }
</script>

<style scoped>

</style>
